...
class OnlineTokenizedIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            yield from self.tokenize(self.data)
    ...
    def tokenize(self, data, buffer_size: int = 64):
        ...
        def flush():
            ...
            tokenized_x = self.tokenizer(
                x_buffer, add_special_tokens=False, return_attention_mask=False
            )["input_ids"]
            tokenized_y = self.tokenizer(
                y_buffer, add_special_tokens=False, return_attention_mask=False
            )["input_ids"]
            for s, input_ids, label_ids in zip(states, tokenized_x, tokenized_y):
                assert len(input_ids) == len(label_ids), (
                    f"input_ids/labels length mismatch: {len(input_ids)} != {len(label_ids)}. "
                    "Expected aligned x/y token lengths per sample."
                )
                self.states = s
                yield {
        ...
        for sample in data:
            if sample.get('x', None) is None or sample.get('y', None) is None:
                raise ValueError(f"No 'x' or 'y' field found in sample:\n{sample}")
            x_buffer.append(str(sample["x"]))
            y_buffer.append(str(sample["y"]))
            states.append(self.data.state_dict())
            if len(x_buffer) >= buffer_size:
                yield from flush()
                x_buffer, y_buffer, states = [], [], []
        yield from flush()
...
class DataCollatorForLanguageModeling:
    ...
    def __call__(self, examples: List[Union[List[int], Dict[str, Any]]]) -> Dict[str, Any]:
        if not isinstance(examples[0], Dict):
            raise ValueError("Tasklet data collator expects dict examples with 'input_ids' and 'labels'.")
        ...
        if not self.varlen:
            ...
            for example in examples:
                if 'labels' not in example:
                    raise ValueError("Each example must contain 'labels' for tasklet supervised training.")
                input_len = example['input_ids'].size(0)
                label_len = example['labels'].size(0)
                assert input_len == label_len, (
                    f"input_ids/labels length mismatch in batch element: {input_len} != {label_len}"
                )
            ...
            if needs_padding:
                ...
                # Pad inputs with tokenizer; pad labels manually with -100.
                ...
                for example in examples:
                    ...
                    if labels.size(0) < max_len:
                        labels = torch.cat(
                            [labels, torch.full((max_len - labels.size(0),), -100, dtype=torch.long)], dim=0
                        )
                    else:
            ...
            # Replace masked label token with ignore index and then mask padding positions.
            mask_id = self.tokenizer.mask_token_id
            batch['labels'][batch['labels'] == mask_id] = -100
            ...
            batch['labels'] = torch.cat([example['labels'] for example in examples], dim=0).unsqueeze(0)
            assert batch['input_ids'].size(1) == batch['labels'].size(1), (
                f"input_ids/labels length mismatch for varlen batch: "
                f"{batch['input_ids'].size(1)} != {batch['labels'].size(1)}"
            )
            mask_id = self.tokenizer.mask_token_id
            batch['labels'][batch['labels'] == mask_id] = -100
...
def build_dataset(
    ...
    if len(dataset.split(',')) == 1:
        ...
        for i, prob in enumerate(data_probs):
            ...
            if "x" in subset.column_names and "y" in subset.column_names:
                subset = subset.select_columns(["x", "y"])
            else:
                raise ValueError(
                    f"Subset {datasets[i]} has no required 'x'/'y' columns"
                )
    ...
    if "x" in dataset.column_names and "y" in dataset.column_names:
        dataset = dataset.select_columns(["x", "y"])
    else:
        raise ValueError(f"Dataset {dataset} has no required 'x'/'y' columns")
