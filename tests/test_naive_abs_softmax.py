...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is required for triton abs softmax attention")
@pytest.mark.parametrize("head_first", [False, True])
@pytest.mark.parametrize("mode", [1, 2])
@pytest.mark.parametrize("dtype", _cuda_dtypes())
def test_naive_abs_softmax_matches_reference(head_first: bool, mode: int, dtype: torch.dtype):
    torch.manual_seed(0)
    ...
    scale = d ** -0.5
    triton_fn = parallel_abs_softmax_2_attn if mode == 2 else parallel_abs_softmax_1_attn
    ref_fn = reference_naive_abs_softmax_2_attn if mode == 2 else reference_naive_abs_softmax_1_attn
...
@pytest.mark.parametrize("mode", [1, 2])
@pytest.mark.parametrize("dtype", _cuda_dtypes())
def test_naive_abs_softmax_backward_matches_reference(head_first: bool, mode: int, dtype: torch.dtype):
    torch.manual_seed(1)
    device = torch.device("cuda")
    b, t, h, d = 1, 7, 2, 8
