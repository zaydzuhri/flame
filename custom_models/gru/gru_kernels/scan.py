...
from .naive import gru_step_torch
from .triton_gru import gru_step_triton
...
def gru_scan(
    ...
    """
    Run the recurrent GRU loop over a full sequence.

    Args:
        hidden_states: [batch, seq_len, hidden]
        initial_state: [batch, hidden] or None
        x_proj: input-to-gates projection, maps hidden -> 3 * hidden
        h_proj: hidden-to-gates projection, maps hidden -> 3 * hidden
    ...
    if initial_state is None:
        ...
        h_prev = initial_state.to(dtype=hidden_states.dtype, device=hidden_states.device)
    ...
    for t in range(seq_len):
        ...
        if _use_triton_step(x_t, h_t, prefer_triton):
            activated = gru_step_triton(x_t.contiguous(), h_t.contiguous())
            if activated is not None:
                r_t, z_t, n_pre = activated.chunk(3, dim=-1)
                n_t = torch.tanh(n_pre)
                h_next = (1.0 - z_t) * n_t + z_t * h_prev
            else:
                h_next = gru_step_torch(x_t, h_t, h_prev)
        else:
            h_next = gru_step_torch(x_t, h_t, h_prev)
