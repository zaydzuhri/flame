...
if _HAS_TRITON:
    ...
    def _lstm_step_kernel(
        ...
        in_g = (idx_in_row >= (2 * hidden_size)) & (idx_in_row < (3 * hidden_size))

        sig = 1.0 / (1.0 + tl.exp(-gate))
        out_gate = tl.where(in_g, gate, sig)
        tl.store(out_ptr + offsets, out_gate, mask=mask)


def lstm_step_triton(x_t: torch.Tensor, h_t: torch.Tensor) -> Optional[torch.Tensor]:
    """
    Optional gate activation kernel.

    Returns:
        Activated [i, f, g_pre, o] tensor with same shape as x_t/h_t,
