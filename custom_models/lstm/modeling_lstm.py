...
from fla.modules import GatedMLP as LSTMMLP
from fla.modules import RMSNorm

from .config_lstm import LSTMConfig
from .lstm_layer import LSTMMixer
...
class LSTMBlock(nn.Module):
    def __init__(self, config: LSTMConfig, layer_idx: int):
        super().__init__()
...
class LSTMModel(LSTMPreTrainedModel):
    def __init__(self, config: LSTMConfig):
        ...
        self.layers = nn.ModuleList([LSTMBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[Any],
        ...
        if output_attentions:
            warnings.warn(
                "`LSTMModel` does not return attention weights, so `output_attentions` is set to `False`."
            )
