...
def compute_sink_sums(
    ...
    if k_len > 1:
        entropy_sum = entropy_sum / math.log(k_len)
    return sink_sum, entropy_sum, valid_tokens
...
class SinkAttentionAccumulator:
    ...
    def update(self, attentions: Sequence[torch.Tensor]) -> None:
        ...
        for idx, attn in enumerate(attentions):
            sink_sum, entropy_sum, valid_tokens = compute_sink_sums(
                attn, self.attention_mask, self.sink_index, self.entropy_eps
            )
...
def compute_dormant_summary(
    ...
    entropy_rates_by_step: Sequence[list[list[float]]] | None,
    ...
    for step_idx, step in enumerate(steps):
        step_rates = sink_rates_by_step[step_idx]
        step_entropy = entropy_rates_by_step[step_idx] if entropy_rates_by_step else None
        ...
        for layer_idx, layer_rates in enumerate(step_rates):
            ...
            for head_idx, rate in enumerate(layer_rates):
                entropy_ok = True
                if entropy_threshold is not None and step_entropy is not None:
                    entropy_ok = step_entropy[layer_idx][head_idx] < entropy_threshold
                is_dormant = rate > dormant_threshold and entropy_ok
                is_mostly = rate > mostly_dormant_threshold and entropy_ok
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dormant-threshold", type=float, default=0.9)
...
def run_checkpoint(
    ...
    return accumulator.sink_attention_rates(), accumulator.entropy_rates(), metadata
...
def main() -> None:
    ...
    for step in steps:
        ...
        sink_rates, entropy_rates, metadata = run_checkpoint(
    ...
    summary = compute_dormant_summary(
        ...
        entropy_rates_by_step=entropy_rates_by_step,
        entropy_threshold=args.entropy_threshold,
    )

    results = {
        "analysis_type": "head_dormant",
