...
def collect_attention_modules(model: torch.nn.Module) -> list[LayerSpec]:
    ...
    for name, module in model.named_modules():
        if isinstance(module, (Attention, StochasticSoftpickAttention)):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Measure per-head off-rate across Softpick checkpoints."
    )
    parser.add_argument(
        "--model-template",
        required=True,
...
def main() -> None:
    ...
    output_path = resolve_output_path(
        args.output, args.output_dir, model_label, suffix="head_dead"
    )
